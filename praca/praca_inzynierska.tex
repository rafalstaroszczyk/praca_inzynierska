...
\usepackage{uarial}
\usepackage[top=2.5cm,bottom=2.5cm,inner=3.5cm,outer=2.5cm, showframe]{geometry}
\usepackage{fancyhdr}
...
\begin{document}
	...
	\end{center}
	W reakcji tej obserwujemy, że stężenia \ce{Br-} oraz \ce{HBrO2}, który nie jest obecny w przedstawionych wyżej sumarycznych równaniach, zmieniają się oscylacyjnie. Równocześnie stężenia \ce{Ce^{3+}} oraz \ce{Ce^{4+}} też podlegają oscylacjom. Oscylacje tego reagenta powoduje widoczne zmiany koloru roztworu w trakcie reakcji z bezbarwnego na żółty, a później ponownie na bezbarwny. Całkowita reakcja sumaryczna ma postać: 
	\begin{center}
	...
	W późniejszym okresie znaleziono również inne reagenty, dla których zachodzi reakcja analogiczna. Jedną z możliwych modyfikacji tej reakcji jest zamiana jonów \ce{Ce^3+} na \ce{Fe(o-phen)3^2+}, zwanych ferroiną. Podmiana ta zmienia efekty wizualne w czasie przebiegu reakcji oscylacyjnej. Zamiast oscylacji między bezbarwnym i żółtym roztworem występuje oscylacja między pomarańczowym oraz niebieskim \cite{orlik}. \par
	...
	We wcześniejszym podrozdziale opisaliśmy zmiany entropii układu, które dokonuje się w trakcie procesu samorzutnego. Jeżeli zmiany te odniesiemy do bardzo krótkiego przedziału czasu \(\dd{t}\), wówczas szybkość zmiany entropii dana będzie za pomocą równania \cite{mazur}:
	\begin{equation}
	...
	\end{equation}
	gdzie $X_{k}$ i $J_{k}$ są skalarami, składowymi wektorów lub składowymi tensorów (dla przepływu lepkiego). 
